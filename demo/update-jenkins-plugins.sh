...
JENKINS_URL=${JENKINS_URL:-http://localhost:8080/jenkins/}
...
JENKINS_COMPOSE_SERVICE_NAME=jenkins
...
docker-compose exec ${JENKINS_COMPOSE_SERVICE_NAME} curl -L -s -S -o "${JENKINS_CLI_PATH}" \
  "${JENKINS_PRIVATE_URL}/jnlpJars/jenkins-cli.jar"

# We fetch the list of plugins
docker-compose exec ${JENKINS_COMPOSE_SERVICE_NAME} java -jar ${JENKINS_CLI_PATH} \
